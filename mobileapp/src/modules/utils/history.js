...
export const getHistoryById = (params, token) => {
  const urlgetHistoryById =
    urlHistory + `?by=create_at&order=desc&limit=4&page=${params.page}`;
  return axios.get(urlgetHistoryById, {headers: {'x-access-token': token}});
};

export const getHistoryByIdOwner = (params, token) => {
  const urlgetHistoryByIdOwner =
    urlHistory +
    `/renter?search=${params.search}&filter=${params.filter}&by=${params.by}&order=${params.order}&limit=${params.limit}&page=${params.page}`;
  return axios.get(urlgetHistoryByIdOwner, {
...
export const postAddOrEditHRating = (id, body, token) => {
  ...
  return axios.patch(urlPostAddOrEditRating, body, {
    headers: {'x-access-token': token},
  });
};
...
export const delHistoryOwner = (body, token) => {
  ...
  return axios.delete(urlDelHistoryOwner, {
    headers: {'x-access-token': token},
    data: {id: body},
  });
};
